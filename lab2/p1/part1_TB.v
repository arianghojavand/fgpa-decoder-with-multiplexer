...
module part1_TB;
	...
	four_bit_2to1mux instantiate_v1(.sel(select),.a(input1),.b(input2),.chosen(output1));
	four_bit_2to1muxV2 instantiate_v2(.sel(select),.a(input1),.b(input2),.chosen(output2));
 	
	initial begin 

		$dumpfile("p1_wave.vcd");
        $dumpvars(0, part1_TB);

        $display("time\tcount\ts\tx\ty\tm\tmV2");
        $monitor("%0t\t%b\t%b\t%b\t%b\t%b\t%b", $time, count, select, input1, input2, output1, output2);
